...
tcltest::test 001 {should be able to route a flit} -body {
	...
	foreach var {nocsim::nocsim_backrouted nocsim::nocsim_routed nocsim::nocsim_spawned nocsim::nocsim_injected nocsim::nocsim_dequeued nocsim::nocsim_arrived} {
		conswrite "\t$var = [subst $$var]"
	}
	...
	if { $backrouted != $nocsim::nocsim_backrouted } { return 700;  }
	if { $routed     != $nocsim::nocsim_routed     } { return 800;  }
	if { $spawned    != $nocsim::nocsim_spawned    } { return 900;  }
	if { $injected   != $nocsim::nocsim_injected   } { return 1000; }
	if { $dequeued   != $nocsim::nocsim_dequeued   } { return 1100; }
	if { $arrived    != $nocsim::nocsim_arrived    } { return 1200; }
