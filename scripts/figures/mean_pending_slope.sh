...
pending="$(awk -f scripts/getheaders.awk < out/results.tsv | grep "mean change in pending flits per tick per PE" | cut -f2)"

gnuplot \
	-e "$GP_TERMINAL" \
	-e 'set datafile separator "\t"' \
	-e 'set key outside below' \
	-e 'set title "Mean Change in Pending Flits Per PE vs Injection Rate"' \
	-e 'set xlabel "injection rate (flit-PEs per cycle)"' \
	-e 'set ylabel "change pending flits (flits per cycle per PE)"' \
...
'out/results_utorus_ADOR.tsv' using $injrate:$pending with linespoints title \"undirected, ADOR\"" \
> out/mean_pending_slope.$GP_EXT
